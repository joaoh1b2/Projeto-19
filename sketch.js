...
function preload(){
  ...
  gasImage = loadImage("obstacle3.png");
}
...
function draw() {
  ...
  if(gasG.isTouching(mainCyclist)){
    gameState = END;
    gasG.destroyEach();
   }
   ...
   if(pinkCG.isTouching(mainCyclist)){
    ...
    text("Clique a Seta pra Cima para Reiniciar",470,200);
...
function obstacleSpawn(){
  gas =createSprite(1100,Math.round(random(50, 250)));
  gas.scale =0.1;
  gas.velocityX = -(6 + 2*distance/150);
  gas.addImage(gasImage);
  gas.setLifetime=170;
  gasG.add(gas);
}
